 ...
 * Take the test
 */
Route::group(['prefix' => 'take-the-test', 'middleware' => ['auth:api']], function () {
    // Start the test
    Route::get('{slug}', 'TestsController@takeTheTest')->name('apiUser_takeTheTest_test');

    // Check user take the test (start the test) or yet
    Route::group(['middleware' => ['takeTheTest']], function () {

        // View MTP question group by part
        Route::get('{slug}/{partId}', 'Home\TestController@getTakeATest1')->where('partId', '[0-9]+')->name('apiUser_mtp_part_detail');

        // Get Coding Problem
        Route::get('{slug}/{partId}/coding/{problemId}', 'CodingProblemsController@showForTakeTheTest')->name('apiUser_coding_detail');

        // Submit
        Route::post('{slug}', 'TestsController@submit')->name('apiUser_test_submit');
    });

    // Old submission by part
    // Route::post('{slug}/{partId}/mtp-choice/{problemId}', "MtpUserAnswersController@saveUserAnswers")->name('apiUser_save_User_Answers');
